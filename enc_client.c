 ...
 * 
 * Description: Program connects to enc_server and has enc_server perform a one-
 *      time pad style encryption using an alphashift method. This program provides
 *      the server with the plaintext file, key, and port that should connect to
 *      enc_server. The enc_server will send back ciphertext and enc_client will
 *      output this via stdout.
***********************************************************************************/
...
int main(int argc, char *argv[]) {
    ...
    //Open the plaintext file passed in as argument for reading
    FILE *plaintext = fopen(argv[1], "r");
    if(plaintext == 0) {
        error("Error: could not open plaintext file", 1);
    }
    //Get input from plaintext, truncate to buffer -1 leaving \0
    ...
    for(i = 0; buffer[i] != '\0'; i++) {
        ...
        if(!valid) {
            error("Error: plaintext file contains bad character", 1);
        }
    }
    ...
    //Send signal that this is for encrypting not decrypting
    do {
        charsWritten = send(socketFD, "e:", strlen("e:"), 0);
        if(charsWritten < 0) {
    ...
    if(buffer[0] == '$') {
        fprintf(stderr, "Error: could not connect to enc_server on port %d\n", portNum);
        exit(2);
    }
